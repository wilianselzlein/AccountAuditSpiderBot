//https://github.com/tebeka/selenium/pull/238/commits/63839f35ddf40471982a4003cc0f62338cae1c2e
package spider
...
func Select(el selenium.WebElement) (se SelectElement, err error) {
	se = SelectElement{}
...
func (s SelectElement) GetElement() selenium.WebElement {
	return s.element
}
...
func (s SelectElement) GetOptions() ([]selenium.WebElement, error) {
	return s.element.FindElements(selenium.ByTagName, "option")
}

// GetAllSelectedOptions Returns all of the options of that Select that are selected
func (s SelectElement) GetAllSelectedOptions() ([]selenium.WebElement, error) {
	// return getOptions().stream().filter(WebElement::isSelected).collect(Collectors.toList());

	var opts []selenium.WebElement
	return opts, nil
}

// GetFirstSelectedOption Returns the first selected option of the Select Element
func (s SelectElement) GetFirstSelectedOption() (opt selenium.WebElement, err error) {
...
func (s SelectElement) SelectByVisibleText(text string) error {
	...
	options, err := s.element.FindElements(selenium.ByXPATH, `.//option[normalize-space(.) = "`+escapeQuotes(text)+`"]`)
	if err != nil {
		return err
	}
	...
	if !matched && strings.Contains(text, " ") {
		...
		var candidates []selenium.WebElement
		if subStringWithoutSpace == "" {
			// hmm, text is either empty or contains only spaces - get all options ...
			candidates, err = s.GetOptions()
		} else {
			// get candidates via XPATH ...
			candidates, err = s.element.FindElements(selenium.ByXPATH, `.//option[contains(., "`+escapeQuotes(subStringWithoutSpace)+`")]`)
		}
...
func (s SelectElement) findOptionsByValue(value string) (opts []selenium.WebElement, err error) {
	opts, err = s.element.FindElements(selenium.ByXPATH, `.//option[@value = "`+escapeQuotes(value)+`"]`)
	if err != nil {
		return
	}
	if len(opts) == 0 {
...
func (s SelectElement) setSelectedByIndex(index int, selected bool) error {
	...
	opts, err := s.element.FindElements(selenium.ByXPATH, `.//option[@index = "`+idx+`"]`)
	if err != nil {
		return err
	}
	if len(opts) == 0 {
...
func (s SelectElement) setSelected(option selenium.WebElement, selected bool) (err error) {
